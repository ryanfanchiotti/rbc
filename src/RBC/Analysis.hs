...
analyzeExpr dv e
    ...
    | (FloatT _) <- e = Right e
    | (StringT _) <- e = Right e
    | (Neg ne) <- e = Neg <$> analyzeExpr dv ne
    | (Addr ne) <- e = Addr <$> analyzeExpr dv ne
    | (Deref ne) <- e = Deref <$> analyzeExpr dv ne
    | (IncL ne) <- e = IncL <$> analyzeExpr dv ne
    | (IncR ne) <- e = IncR <$> analyzeExpr dv ne
    | (DecL ne) <- e = DecL <$> analyzeExpr dv ne
    | (DecR ne) <- e = DecR <$> analyzeExpr dv ne
    | (Not ne) <- e = Not <$> analyzeExpr dv ne
